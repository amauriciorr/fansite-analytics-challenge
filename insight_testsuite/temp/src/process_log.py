...
#for some reason I was encoutering a decoding error with the text file--specifically with byte 0x80
...
#first run through reveals that most used are probably images upon loading the webpage, so we ignore .gif files
# /, 687452
...
#second run, after ignoring gifs, we see that .xbm's are popular. most likely icon images, so we ignore those too
 # /, 1421896
 # /ksc.html, 162816
